...
const Dolor = () => {
    ...
    const {auth,actualizarDolor} =  usePreguntasCli()
    ...
    const handleSubmit = async e =>{
        e.preventDefault()
        await  actualizarDolor(perfil)
        SetOcultarSeccion(true)
        
       }
       const SiDolor= async e =>{
        e.preventDefault()
        await  actualizarDolor(perfil2)
...
<div className="inline-block mr-1 mt-1">
  <div className="flex">
  <div>
    <input type="radio" name="dolor" id="sidolor" className="peer hidden border border-gray-200" value='Si'
...
<div className="flex">
<div>
<input type="radio" name="dolor" id="nodolor" className="peer  hidden border border-gray-200" value='No'
          onChange={ e => setPerfil({
           ...perfil,
           [e.target.name] : e.target.value
         })} />
...
<div className='text-sm'>
  ...
  <h1 className=' font-nunito'> { auth.historiaclinica?.dolor==='Si' ?
    <div className=''>
    <h1 className='font-bold text-[12px] '> ¿Siente dolor u otra molestia frecuentemente? </h1>
   ...
   <div className=''>
    <h1 className='font-bold text-[12px] '>¿Siente dolor u otra molestia frecuentemente? </h1>
    <span className=' font-bold text-green-700'> No </span>
    <div className='flex justify-center'>  <span className='text-2xl'>😃</span></div>
    </div>
   } </h1>    
  </div>
<form onSubmit={SiDolor}  >
<input type="radio" name="dolor" className="peer hidden " value='Sin datos' checked
