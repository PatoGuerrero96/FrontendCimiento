...
const FormularioActividadFisica = () => {
    const { id } = useParams();
    ...
    const config = {
     ...
     setConsulta(data)
      ...
      const handleChange = (e) => {
        ...
        // Si el campo es 'actividadfisica', actualiza solo esa propiedad dentro de historiaclinica
        if (name === 'actividadfisica') {
      ...
      const CamposVacios =
      !datosPaciente.obsactividadfisica ||
      datosPaciente?.historiaclinica?.actividadfisica==='Sin datos';
     const nombreVacio = !datosPaciente.obsactividadfisica;
     const actividadVacia = datosPaciente?.historiaclinica?.actividadfisica ==='Sin datos' || '';
    ...
    <>
        ...
        <div className="flex justify-start gap-2 py-1 px-4 rounded-t-lg bg-lila-300">
              <div>
                ...
                Actividad física:  {datosPaciente?.historiaclinica?.actividadfisica} 
                </h1>
              </div>
              <div>
                <button
...
<div className="flex justify-start gap-2">
  ...
  {datosPaciente?.historiaclinica?.actividadfisica} 
  </h2>
    </div>
...
{ocultarActividad ? null : (
  ...
  <div className="grid grid-cols-1 sm:grid-cols-1 ">
    ...
    <div className="flex flex-col text-sm">
      <div className="flex">
      <label htmlFor="obsactividadfisica" className="text-md  ">Describe la actividad fisica del paciente:</label>
      {nombreVacio && (
    ...
    <textarea
        ...
        name="obsactividadfisica"
        value={datosPaciente.obsactividadfisica || ''}
        onChange={handleChange}
    />
    </div>
    <div className="flex flex-col text-sm">
      <div className="flex">
      <label htmlFor="obsactividadfisica" className="text-md  ">¿Realiza actividad física?</label>
      {actividadVacia && (
              <span className="text-red-500 text-md"><FaExclamation className="animate-pulso text-lg" /></span>
                )}
    </div>
  ...
  value={datosPaciente.historiaclinica.actividadfisica || ""}
  onChange={handleChange}
>
  ...
  <option value="No">No</option>
      ...
      <div className="flex items-center  gap-1">
        <label htmlFor="nombre" className="font-bold">Actividad fisica del paciente:</label>
        <label>{datosPaciente.obsactividadfisica || ''} </label>
      </div>
      <div className="flex items-center  gap-1">
        <label htmlFor="ultimocontrol" className="font-bold">
        ¿Realiza actividad física?:
        </label>
        <label>{datosPaciente.historiaclinica.actividadfisica ||''}</label>
      </div>
    </div>
  )}
...
export default FormularioActividadFisica