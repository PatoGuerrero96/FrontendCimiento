...
const Signosdealarma = () => {
    const {authpro} =  proAuth()
    const [nombre, setNombre] = useState('');
    const [fuente, setFuente] = useState('');
    const [contenido, setContenido] = useState([]);
    const [anonimo, setAnonimo] = useState(false);
    const [signosalarmas, setSignosalarma] = useState([]);
    ...
    const maximo = Math.ceil(signosalarmas.length / porPagina);
    ...
    const obtenerRecetasMagistral = async () => {
      try {
        ...
        const { data } = await clientAxios.get(`/profesional/obtener-tus-signos/${authpro._id}`,config);
        setSignosalarma(data);
      } catch (error) {
        console.log(error);
      }
    };
    const eliminarRecetaMagistral = async (id) => {
      ...
      const confirmar = await Swal.fire({
        title: '¿Estás seguro de que quieres eliminar tus datos de signos de alarma?',
        icon: 'warning',
      ...
      try {
        ...
        await clientAxios.delete(`/profesional/eliminar-signos/${id}`,config);
        obtenerRecetasMagistral();
      } catch (error) {
        console.log(error);
      }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!nombre) {
          Swal.fire('¡Error!', 'Por favor, Agregue un nombre a tus signos de alarma', 'error');
          return;
        }
        if (!fuente) {
          Swal.fire('¡Error!', 'Por favor, Agregue una fuente para los datos de signos de alarma', 'error');
          return;
        }
        if (!contenido) {
          Swal.fire('¡Error!', 'Por favor, Agregue el contenido de sus signos de alarma', 'error');
          return;
        }
        const confirmar = await Swal.fire({
          title: '¿Estas seguro de publicar estos datos para signos de alarma?',
          icon: 'info',
          showCancelButton: true,
        ...
        await clientAxios.post(`/profesional/guardar-datos-signos`, data,config);
          // Mostrar la notificación de éxito
          Swal.fire('¡Perfecto!', 'Datos de signos de alarma guardados', 'success');
          obtenerRecetasMagistral();
        } catch (error) {
          console.log(error);
          // Mostrar la notificación de error
          Swal.fire('¡Error!', 'Ocurrió un error al guardar los datos de signos de alarma', 'error');
        }
      }
      setNombre('')
      setFuente('')
      ...
      const guardarEdicion = () => {
        ...
        if (!datosEditar.nombre) {
          ...
          Swal.fire('¡Error!', 'Guarde un nombre para los datos de signos de alarma', 'error');
          return;
        }
        ...
        if (!datosEditar.contenido || datosEditar.contenido.length === 0) {
            ...
            Swal.fire('¡Error!', 'Ingrese contenido para los datos de signos de alarma', 'error');
            return;
          }
        ...
        clientAxios
          .put(`/profesional/actualizar-signos/${datosEditar._id}`, data, config)
          .then((response) => {
            // Actualizar el estado con los datos editados
            const recetasActualizadas = signosalarmas.map((me) => {
              if (me._id === datosEditar._id) {
                return { ...me, ...data };
              }
              return me;
            });
            Swal.fire('¡Perfecto!', 'Signos de alarma actualizados', 'success');
            setSignosalarma(recetasActualizadas);
            setEditarModalAbierto(false);
          })
          .catch((error) => {
            Swal.fire('¡Error!', 'Signos de alarma no se pudierona ctualizar', 'error');
          });
      };
  ...
  <div className="md:col-span-2 lg:col-span-1">
    ...
    <div className="max-w-md mx-auto bg-slate-200 p-4 shadow-lg">
      <h1 className="text-2xl font-bold mb-4">Guardar Datos de signos de alarma</h1>
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <label htmlFor="nombre" className="block mb-2">
            Nombre del signo de alarma:
          </label>
          <input
...
<div className="flex justify-center">
  ...
  <div>
    <h1 className="font-bold">Tus Datos para signos de alarma</h1>
    {signosalarmas.length === 0 ? (
      <p>Aún no has subido datos para signos de alarma.</p>
    ) : (
<div className="divide-y divide-gray-300">
  {signosalarmas.slice((pagina - 1) * porPagina, (pagina - 1) * porPagina + porPagina).map((me) => (
...
{editarModalAbierto && (
  ...
  <div className="bg-white rounded-lg p-6">
    <h2 className="text-lg font-bold mb-4">Editar signos de alarma</h2>
    {/* Campo de título */}
  ...
  {datosEditar.contenido.map((value, index) => (
    <div key={index} className="flex items-center mb-2">
      ...
      <div className="flex justify-center">
      <button
    className="bg-lila-300 text-white hover:bg-lila-200 px-4 py-2 rounded-lg text-center"
